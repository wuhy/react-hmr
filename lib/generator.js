...
function amdWrap(mod, modPathIdMap, ignoreAMDWrapFileMap) {
    var code = mod.content;
    var modFilePath = mod.file;
    var modId = getModId(modFilePath);
    ...
    if (!/^\s*define\s*\(\s*/.test(tmp) && !ignoreAMDWrapFileMap[modFilePath]) {
        var deps = ['require', 'exports', 'module'].concat(mod.deps);
        // console.log(defineModId + ' deps: \n')
        // console.log(JSON.stringify(deps) + '\n\n');
        code = 'define(\'' + defineModId + '\', ' + JSON.stringify(deps) + ', function (require, exports, module) {'
            + code + '\n});\n';
    }

    // generate the proxy module
    for (var i = 1, len = usedIds.length; i < len; i++) {
        if (usedIds[i] !== defineModId) {
            code += '\ndefine(\'' + usedIds[i] + '\', '
                + JSON.stringify(['require', 'exports', 'module', defineModId])
...
module.exports = exports = function (options) {
    ...
    while (entries.length) {
        ...
        var syncDeps = [];
        var synDepLen = depInfo.syncDeps.length;
        for (var i = 0, len = allDeps.length; i < len; i++) {
            ...
            if (depFilePath) {
                ...
                if (i < synDepLen && syncDeps.indexOf(requireId) === -1) {
                    syncDeps.push(requireId);
                }
            }
        }
    ...
    result = result.map(function (item) {
        return amdWrap(
            item, modPathIdMap, ignoreAMDWrapFileMap
        );
    });
